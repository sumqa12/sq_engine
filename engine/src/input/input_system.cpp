...
void update_camera_control(const ecs::Registry& registry, const InputManager& input, float dt) {
    // FreeFly カメラ操作（phase9プラン）
    registry.view<scene::ControlTarget, scene::Controller, scene::Camera>().each(
        [&](ecs::Entity, scene::ControlTarget, scene::Controller& ctrl, scene::Camera& cam) {

            // 1. マウス視線（右ボタン押下中のみ）
            if (!input.is_down(GLFW_KEY_LEFT_ALT)) {
                auto [dx, dy] = input.cursor_delta();
                ctrl.yaw   += static_cast<float>(dx) * ctrl.mouse_sensitivity;
                ctrl.pitch -= static_cast<float>(dy) * ctrl.mouse_sensitivity;
            ...
            if (input.is_down(GLFW_KEY_SPACE)) cam.position += world_up * step;
            if (input.is_down(GLFW_KEY_LEFT_CONTROL)) cam.position -= world_up * step;
